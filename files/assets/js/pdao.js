// Data structure with PDAO count
const barangayData = {
    1: { name: "Barangay 1", puroks: ["Kamagong", "Narra", "Ipil-Ipil", "Akasya", "Flying-E"], pdaoCount: 156 },
    2: { name: "Barangay 2", puroks: ["Gomez", "Katipunan", "Kahilwayan", "Sool Uno", "Sool Dos"], pdaoCount: 124 },
    3: { name: "Barangay 3", puroks: ["Paghida.et A", "Paghida.et B", "Ilimnan", "Guintipunan", "Mahigugmaon"], pdaoCount: 98 },
    4: { name: "Barangay 4", puroks: ["Antilla Subd", "Gomez Extension", "Bonifacio Extension", "Mckinley Bukid", "Zulueta Bukid"], pdaoCount: 167 },
    5: { name: "Barangay 5", puroks: ["Elina subd", "Portuna", "St francis", "Carmilla paste 3", "Villa carmen"], pdaoCount: 119 },
    6: { name: "Barangay Mambulac", puroks: ["Paghidaet", "Antoni Luna", "Swimming Pool", "Boulevard", "Barra"], pdaoCount: 187 },
    7: { name: "Barangay Guinhalaran", puroks: ["Katilingban", "Sawmill", "Paghidait", "Mangingisda", "Baybayon"], pdaoCount: 136 },
    8: { name: "Barangay E-Lopez", puroks: ["Sunshine", "Sunrise", "Sunset", "Sampaguita", "Newsite"], pdaoCount: 145 },
    9: { name: "Barangay Bagtic", puroks: ["Proper", "New site", "Bactic uno", "Kalbaryo", "Defuigo"], pdaoCount: 112 },
    10: { name: "Barangay Balaring", puroks: ["Camunsilan", "Proper", "Bungol", "Hda Balaring", "Pasil"], pdaoCount: 158 },
    11: { name: "Barangay Hawaiian", puroks: ["Colisap", "Phison", "balas", "Lunot", "Sandiego"], pdaoCount: 94 },
    12: { name: "Barangay Patag", puroks: ["Mahigugmaon", "Malipayun", "Mainabyanon", "Marka"], pdaoCount: 127 },
    13: { name: "Barangay Kapt. Ramon", puroks: ["Hda.Adoracion", "Hda.Boac", "Hda.Progreso", "Hda.Banita jarra", "Hda.Violata"], pdaoCount: 165 },
    14: { name: "Barangay Guimbalaon", puroks: ["Proper", "New Site", "Bactic Uno", "Kalbaryo", "Defuigo"], pdaoCount: 129 },
    15: { name: "Barangay Rizal", puroks: ["Matagoy", "Paradise", "Kalubihan", "Baryo Rizal", "Hda Makina"], pdaoCount: 173 },
    16: { name: "Barangay Lantad", puroks: ["Mapisanon", "Nami nami", "Bay-bay", "Paraiso", "Mainuswagon"], pdaoCount: 108 }
};
...
// Utility Functions
const utils = {
    calculatePurokPdaoCounts: (barangayId) => {
        const data = barangayData[barangayId];
        const purokCount = data.puroks.length;
        const baseCount = Math.floor(data.pdaoCount / purokCount);
        const remainder = data.pdaoCount % purokCount;
    ...
    calculateSummaryStats: () => {
        ...
        Object.entries(barangayData).forEach(([id, data]) => {
            totalPdaos += data.pdaoCount;
            if (data.pdaoCount > highestCount) {
                highestCount = data.pdaoCount;
                highestBarangay = data.name;
            }
        });
        
        const barangayCount = Object.keys(barangayData).length;
        const avgPdaos = Math.round(totalPdaos / barangayCount);
        
        document.getElementById('total-barangays').textContent = barangayCount;
        document.getElementById('total-seniors').textContent = totalPdaos;
        document.getElementById('avg-seniors').textContent = avgPdaos;
        document.getElementById('highest-count').textContent = `${highestCount} (${highestBarangay})`;
        
        return { totalPdaos, barangayCount, avgPdaos, highestCount, highestBarangay };
    }
};
...
const core = {
    filterBarangays: (searchTerm = '') => {
        state.filteredBarangays = Object.entries(barangayData)
            ...
            .sort((a, b) => b[1].pdaoCount - a[1].pdaoCount);
    ...
    populatePurokTable: (barangayId) => {
        const purokData = utils.calculatePurokPdaoCounts(barangayId);
        elements.purokTableBody.innerHTML = '';
        
        const totalPdaos = purokData.reduce((sum, purok) => sum + purok.count, 0);
        
        purokData.forEach(purok => {
            const percentage = totalPdaos > 0 ? ((purok.count / totalPdaos) * 100).toFixed(1) : 0;
            const row = document.createElement('tr');
            row.innerHTML = `
                <td>${purok.name}</td>
                <td>${purok.count}</td>
        ...
        state.purokModalChart = new Chart(elements.purokModalChart, {
            ...
            options: {
                ...
                plugins: {
                    ...
                    title: {
                        display: true,
                        text: `PDAO Distribution in ${data.name}`,
                    ...
                    tooltip: {
                        ...
                        callbacks: {
                            ...
                            label: context => {
                                ...
                                return `PDAO Members: ${value} (${percentage}%)`;
                            }
                        }
                    }
                },
    ...
    createBarangayChart: () => {
        ...
        const data = sortedData.map(([id, barangay]) => barangay.pdaoCount);
        ...
        state.barangayChart = new Chart(ctx, {
            ...
            options: {
                ...
                maintainAspectRatio: false,
                plugins: {
                    title: { display: true, text: 'PDAO Members per Barangay', font: { size: 16 } },
                    legend: { display: false },
                    tooltip: {
                        callbacks: {
                            label: context => `PDAO Members: ${context.raw}`
                        }
                    }
                },
                scales: {
                    y: { beginAtZero: true, title: { display: true, text: 'Number of PDAO Members' } },
