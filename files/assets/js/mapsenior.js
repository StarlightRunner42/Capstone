define([
  ...
  const barangays = [
  { name: "Bagtic", lat: 10.76204, lon: 123.05122, seniorCount: 45, population: 2850 },
  { name: "Balaring", lat: 10.83171, lon: 122.96136, seniorCount: 32, population: 1920 },
  { name: "Barangay I (Poblacion)", lat: 10.80240, lon: 122.97624, seniorCount: 78, population: 4200 }, // ✅ FIXED
  { name: "Barangay II (Poblacion)", lat: 10.79938, lon: 122.97828, seniorCount: 65, population: 3750 },
  { name: "Barangay III (Cinco de Noviembre)", lat: 10.79770, lon: 122.97281, seniorCount: 89, population: 4800 },
  { name: "Barangay IV (Poblacion)", lat: 10.78407, lon: 123.00921, seniorCount: 56, population: 3200 },
  { name: "Barangay V (Poblacion)", lat: 10.78147, lon: 122.99145, seniorCount: 43, population: 2650 },
  { name: "Barangay VI (Hawaiian)", lat: 10.82606, lon: 123.00549, seniorCount: 67, population: 3900 },
  { name: "E Lopez", lat: 10.82060, lon: 123.03538, seniorCount: 29, population: 1800 },
  { name: "Guimbala-on", lat: 10.75730, lon: 123.07857, seniorCount: 38, population: 2300 },
  { name: "Guinhalaran", lat: 10.7811, lon: 122.9666, seniorCount: 52, population: 3100 },
  { name: "Kapitan Ramon", lat: 10.77394, lon: 123.11920, seniorCount: 24, population: 1500 },
  { name: "Lantad", lat: 10.80845, lon: 122.97199, seniorCount: 41, population: 2400 },
  { name: "Mambulac", lat: 10.79754, lon: 122.9679, seniorCount: 35, population: 2100 },
  { name: "Patag", lat: 10.72466, lon: 123.15720, seniorCount: 18, population: 1200 },
  { name: "Rizal", lat: 10.79816, lon: 122.99473, seniorCount: 47, population: 2800 }
];
  ...
  const totalSeniors = barangays.reduce((sum, b) => sum + b.seniorCount, 0);
  ...
  const highestSenior = Math.max(...barangays.map(b => b.seniorCount));
  const highestBarangay = barangays.find(b => b.seniorCount === highestSenior).name;
  ...
  /*
  fetch("/barangay-senior-data")
    .then(res => res.json())
    .then(seniorData => {
      // Enhanced data processing with animations
      console.log("📊 Loading real-time Senior data...");

      seniorData.forEach(data => {
        const existingBarangay = barangays.find(b => b.name === data.name);
        if (existingBarangay) {
          existingBarangay.seniorCount = data.seniorCount;
