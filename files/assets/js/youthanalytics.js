...
async function loadLydoData() {
    try {
        const res = await fetch('/api/analytics/lydo', { credentials: 'same-origin' });
        const json = await res.json();
        if (!json.success) throw new Error('Failed to fetch LYDO data');

        // Map into barangayData with numeric ids as keys
        barangayData = {};
        json.data.forEach(item => {
            barangayData[item.id] = { name: item.name, lydoCount: item.lydoCount };
        });
...
function showChart(barangayId) {
    ...
    const selectedCount = barangay.lydoCount;
    ...
    chartInfo.innerHTML = `
        ...
        <p><strong>LYDO Count:</strong> ${selectedCount.toLocaleString()}</p>
